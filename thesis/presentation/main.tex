...
\setbeamertemplate{frametitle}{%
  ...
  \hbox{%
    ...
    \hfill
    \raisebox{-0.08cm}{\includegraphics[height=0.62cm]{uulm_logo.png}}%
    \hspace{0.95cm}%
  }%
  \vskip0.12cm%
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  % left: GDPR photo (full), vertically centered
  \node[anchor=west,inner sep=0pt] at ([xshift=0.5cm,yshift=-5.45cm]current page.north west)
    {\includegraphics[width=0.455\paperwidth]{GDPR-scaled.jpg}};
  ...
  % right: red panel (matched to the photo's height)
  \fill[uulmred] ([yshift=-3.03cm]current page.north)
    rectangle ([xshift=-0.5cm,yshift=-7.87cm]current page.north east);
  % title text
  \node[anchor=north west,text=white,align=left,text width=0.45\paperwidth,
        font=\bfseries]
    at ([xshift=0.35cm,yshift=-3.35cm]current page.north)
    {\hyphenpenalty=10000\exhyphenpenalty=10000%
     {\fontsize{14}{18}\selectfont Assessing the Effectiveness of Large Language Model Support for Generating GDPR ROPA Documentation}\\[0.45cm]
     {\fontsize{12}{15}\selectfont Master Thesis Presentation}\\[0.3cm]
...
% ============================================================
% 7a -- Research questions overview (all three in focus)
% ============================================================
\begin{frame}
\rqband{\rqbody{white}{white}{white}}
\end{frame}
...
\raggedright\footnotesize \plus speed\\ \plus clarity\\ \plus control\\ \minus LLM least transparent}}
\end{column}
\begin{column}{0.3\textwidth}
\colorbox{uulmred!18}{\parbox[t][2.7cm][t]{\linewidth}{\centering\textbf{2.\ Chat}\\[0.3cm]
\raggedright\footnotesize \plus comfort and offloading responsibility (``LLM takes care of it'') \\ \plus most confident \\ \minus LLM opacity}}
\end{column}
\begin{column}{0.3\textwidth}
\colorbox{uulmgold!30}{\parbox[t][2.7cm][t]{\linewidth}{\centering\textbf{3.\ Ask}\\[0.3cm]
\raggedright\footnotesize \plus high LLM transparency\\ \minus cumbersome workflow}}
\end{column}
\end{columns}
\end{frame}
...
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{ropagen-form.jpg}
\end{column}
\begin{column}{0.36\textwidth}
\scriptsize
\begin{itemize}\setlength{\itemsep}{0.3cm}
  \item \textbf{Scaffolding}: structure beats LLM reasoning.
  \item \textbf{Entry point} for novices, but high recall brings \textbf{overhead} (fields present even when never described).
  \item Users tend to \textbf{overcompensate}.
  \item \textbf{Least transparent} LLM involvement.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{columns}[c]
\begin{column}{0.62\textwidth}\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{ropagen-ask.jpg}
\end{column}
\begin{column}{0.36\textwidth}
\scriptsize
\begin{itemize}\setlength{\itemsep}{0.3cm}
  \item \textbf{Ask fails} the user it was built for: a novice cannot do everything alone (know what to ask).
  \item The LLM must act as a \textbf{co-author}, not wait to be asked.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{columns}[c]
\begin{column}{0.62\textwidth}\centering
\includegraphics[width=\linewidth,height=0.78\textheight,keepaspectratio]{ropagen-chat.jpg}
\end{column}
\begin{column}{0.36\textwidth}
\scriptsize
\begin{itemize}\setlength{\itemsep}{0.3cm}
  \item \textbf{Plays too much into the user} (affirms rather than corrects); trades completeness for grounding: \textbf{risk of omission}.
  \item High confidence \textbf{offloads responsibility} to the LLM.
  \item Works as a \textbf{collaborator}.
  \item Participants were \textbf{chatbot-fluent} (sample caveat).
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{RQ3 --- It depends which quality measure is laid against confidence.} Confidence
        carries no signal against reference-based NLP, matches the judge's holistic Overall score,
        and \emph{inverts} on legal correctness, where the most-trusted mode is rated lowest.
...
\begin{frame}{Perceived AI Support Detail}
\centering
\includegraphics[width=\linewidth,height=0.80\textheight,keepaspectratio]{ai_subquestions_boxplot.png}
\end{frame}
